...
def download_content(folder_dict, session=session, write_mode="wb", dir="songs"):
    for content in folder_dict["files"]:
        ...
        download_url = re.findall(
            r'class="DownloadButtonAd-startDownload gbtnSecondary" href=\'(.*?)\'  onclick="', file_location)
...
@asyncio.coroutine
def download_all(folders):
    futures = []

    for folder in folders["folders"]:
        content_dict = get_song_folder_contents(
            home_folder_url, "files", folder["folderkey"])
        futures.append(loop.run_in_executor(
            None, functools.partial(download_content, content_dict)))

    for req in asyncio.as_completed(futures):
        yield from req


loop.run_until_complete(download_all(folders))
